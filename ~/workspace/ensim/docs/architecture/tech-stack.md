...
**Monorepo tooling:** npm workspaces (`packages/*`, `apps/*`, declared in the root `package.json`) for the TypeScript side — corrected 2026-09-05 from an earlier pnpm plan that was never actually used; `org-doctrine-model`'s validation has been running on plain `npm install`/`npm run validate` since the scaffold was built, and npm ships with Node so there's nothing extra to install. The Java module is *not* forced into the same package manager — a monorepo means one repo and coordinated builds, not one build tool. A root `Justfile` wires `npm` and `mvn` together (`just build`, `just test`, `just dev`) so a single command works across both.
...
npm workspaces + Maven hasn't been validated end-to-end with a real `just build` yet — only `org-doctrine-model`'s own `npm install && npm run validate` has actually been run and passed. Treat the Maven side and the root-level `just build` as the plan going into the first build, not a proven pipeline.
