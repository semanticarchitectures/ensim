...
**Status:** Draft v0.8 — RTI, message format, Mission 1 fidelity, tech stack, `org-doctrine-model` (now with real AOC division data), and the JSON-Schema-vs-BFO/CCO decision are all settled. `model-explorer`'s data-navigation half (Section 13) is built and verified. `federation-kernel` has a working minimal federate (Section 4a), verified end-to-end across two real Portico processes. `sim-services` runs Mission 1's discrete-event timeline end to end and publishes it to a real federation via `federation-kernel` — the full stack now connects. Ready for `model-explorer`'s v2 results view and `c2-interfaces`.
**Date:** 2026-09-07
...
**View mission simulation results.** V1 (built): renders a Mission record's `timeline` array (Section 6/7) as the *planned* sequence. V2 (not yet built, but now unblocked): `sim-services` exists and produces a real `RunResult` — see `packages/sim-services/schema/run-result.schema.json` — per-step status, simulated timestamps, and artifacts generated (an ATO line, a mission report), written to `missions/<id>/run-<timestamp>.json`. The v2 view should render that file, reusing V1's view code fed richer data rather than being rebuilt, per the plan already laid out here.
