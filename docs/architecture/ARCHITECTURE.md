...
**Status:** Draft v1.1 — RTI, message format, Mission 1 fidelity, tech stack, `org-doctrine-model` (now with real AOC division data), and the JSON-Schema-vs-BFO/CCO decision are all settled. `model-explorer` (Section 13) has both its data-navigation half and its v2 results view built and verified. `federation-kernel` has a working minimal federate (Section 4a), verified end-to-end across two real Portico processes. `sim-services` runs Mission 1's discrete-event timeline end to end and publishes it to a real federation via `federation-kernel` — the full stack now connects, and `model-explorer` renders the real run output. `c2-interfaces` (Section 5) is schema-only (five message types for the request → validate → ATO line → execution → report sequence). `ops-dashboard` (Section 10) is built as a static reader — ATO state, C2 node hand-offs, mission timeline — over real Mission 1 data; "live" (subscribing to a running federation) is a deliberate v2, not built.
**Date:** 2026-09-07
...
- **System** — a piece of equipment or infrastructure personnel interact with during mission execution (an aircraft, a communications system, a security/access-control system, an airdrop mechanism). Deliberately representative, never a real fielded system's specifications (AGENTS.md Section 3).
- **Interaction** — a structural (doctrinal, not mission-instance-specific) edge between two nodes — a Role, C2Node, Organization, or System — describing how personnel and systems normally interact during mission execution (operates, communicates via, authenticates to, coordinates with, commands). Sits alongside `DoctrineProcess` as a finer-grained view: individual personnel and equipment, rather than organizational tasking flow.

The initial USAF seed dataset (previously a table in this document) now lives as validated JSON in `packages/org-doctrine-model/data`: `organizations.json` (INDOPACOM, PACAF, PACAF theater AOC, AMC, 618 AOC/TACC, 15th Wing, 535th Airlift Squadron), `roles.json` (CFACC, DIRMOBFOR, Combat Plans Division Chief, Combat Operations Division Chief, Mission Commander, Aircraft Commander, Copilot, Loadmaster, Air Traffic Controller, Drop Zone Support Team Leader), `c2nodes.json` (Wing Operations Center, the five internal AOC divisions — Strategy, Combat Plans, Combat Operations, ISR, Air Mobility — plus a generic AOC tasking node and the 618 AOC/TACC tasking function), `doctrine-processes.json` (the HA/DR request-and-tasking flow, and the ATO cycle), `systems.json` (a representative C-17 airframe, tactical communications system, identity/access-control system, and CDS airdrop release mechanism), and `interactions.json` (9 records connecting Mission 1's crew/ATC/ground/commander personnel to each other and to those four systems). Every record carries a `doctrineSource` citation; several — `pacaf-theater-aoc`, `air-traffic-controller`, `drop-zone-support-team-leader`, and the `System`/`Interaction` records citing AFMAN 13-204 or DoDI 8520.04 — flag an unverified specific fact or an inaccessible primary source rather than guessing it.
